// scripts/deploy-v12.js
const hre = require("hardhat");

async function main() {
    console.log("🚀 Starting DailyAppV12Secured deployment...\\n");
    ...
    // Validate addresses
    if (TOKEN_ADDRESS === "0x0000000000000000000000000000000000000000" || USDC_ADDRESS === "0x0000000000000000000000000000000000000000") {
        console.error("\\n❌ ERROR: TOKEN_ADDRESS or USDC_ADDRESS not set in environment variables!");
        console.log("Please set them in your .env file");
        process.exit(1);
    }

    // Deploy DailyAppV12Secured
    console.log("\\n📦 Deploying DailyAppV12Secured contract...");
    const DailyApp = await ethers.getContractFactory("DailyAppV12Secured");
    const dailyApp = await DailyApp.deploy(TOKEN_ADDRESS, USDC_ADDRESS, INITIAL_OWNER);

    await dailyApp.waitForDeployment();
    const contractAddress = await dailyApp.getAddress();

    console.log("✅ DailyAppV12Secured deployed to:", contractAddress);
    ...
    console.log("\\n🔐 Granting VERIFIER_ROLE to backend service...");
    const VERIFIER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("VERIFIER_ROLE"));
    const tx = await dailyApp.grantRole(VERIFIER_ROLE, VERIFIER_ADDRESS);
    await tx.wait();
    console.log("✅ VERIFIER_ROLE granted to:", VERIFIER_ADDRESS);
    ...
    if (hre.network.name !== "hardhat" && hre.network.name !== "localhost") {
        ...
        try {
            await hre.run("verify:verify", {
                ...
                constructorArguments: [TOKEN_ADDRESS, USDC_ADDRESS, INITIAL_OWNER],
            });
            console.log("✅ Contract verified on Etherscan!");
        } catch (error) {
            console.log("⚠️  Verification failed:", error.message);
            console.log("You can verify manually later with:");
            console.log(`npx hardhat verify --network ${hre.network.name} ${contractAddress} ${TOKEN_ADDRESS} ${USDC_ADDRESS} ${INITIAL_OWNER}`);
        }
    }
    ...
    const fs = require('fs');
    const deploymentPath = `./deployments/${hre.network.name}_v12_deployment.json`;
    ...
    console.log(`   await dailyApp.grantRole(ADMIN_ROLE, "${INITIAL_OWNER}")`);
    console.log("\\n2. 🎨 Set tier URIs:");
    ...
    console.log("\\n3. 📝 Add tasks with verification:");
    console.log("   await dailyApp.addTask(200, 86400, 0, 'Follow on Farcaster', 'https://...', true)");
    console.log("\\n4. 🔧 Configure verification server:");
    console.log(`   CONTRACT_ADDRESS=${contractAddress}`);
    console.log(`   VERIFIER_PRIVATE_KEY=<verifier_wallet_private_key>`);
    console.log("\\n5. 🚀 Deploy verification server to Vercel");
    console.log("\\n6. 📊 Set up monitoring (Tenderly/Defender)");
    console.log("\\n7. 🐛 Launch bug bounty program");
    console.log("\\n8. 📢 Announce deployment to community");
    console.log("\\n" + "=".repeat(60));
