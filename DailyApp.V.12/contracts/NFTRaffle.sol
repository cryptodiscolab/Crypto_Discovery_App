...
contract NFTRaffle is ReentrancyGuard, Ownable, Pausable, RrpRequesterV0 {
    ...
    uint256 public ticketPrice = 0.15 * 10**6; // Default $0.15 (USDC has 6 decimals)
    ...
    uint256 public creatorSharePercentage = 70;  // 70%
    ...
    struct RaffleEvent {
        ...
        bool revenueWithdrawn;   // NEW: Track if revenue has been withdrawn
        bool isActive;
        bool isCompleted;
    ...
    function buyTickets(
        ...
        // Calculate payment
        uint256 totalCost = paidTickets * ticketPrice;
        if (totalCost > 0) {
     ...
     * @param _raffleId ID of the raffle
     */
    function claimPrizes(uint256 _raffleId) external nonReentrant {
        RaffleEvent storage raffle = raffles[_raffleId];
        ...
        require(usdcToken.balanceOf(msg.sender) >= claimFee, "Insufficient USDC for fee");
        usdcToken.safeTransferFrom(msg.sender, address(this), claimFee);
        totalFees += claimFee;
    ...
    /**
     * @notice Set ticket price (Owner only)
    ...
    /**
     * @notice Reset raffle draw request (Owner only)
     * @dev Use this if QRNG request hangs or fails
     * @param _raffleId ID of the raffle
     */
    function resetRaffleDraw(uint256 _raffleId) external onlyOwner {
        RaffleEvent storage raffle = raffles[_raffleId];
        require(raffle.isActive, "Raffle already completed");
        require(raffle.airnodeRequestId != 0, "No pending draw");
        
        // Reset request ID to allow calling drawWinner again
        raffle.airnodeRequestId = 0;
    }
    ...
    function withdrawRaffleRevenue(uint256 _raffleId) external nonReentrant {
        ...
        uint256 revenue = raffle.paidTicketsSold * ticketPrice;
        require(revenue > 0, "No revenue to withdraw");
        
        // Ensure revenue is only withdrawn once
        require(!raffle.revenueWithdrawn, "Revenue already withdrawn");
        raffle.revenueWithdrawn = true;
        
        // Calculate Split
        uint256 platformShare = (revenue * platformSharePercentage) / 100;
        uint256 creatorShare = revenue - platformShare;
        
        // Transfer Creator Share
        if (creatorShare > 0) {
            usdcToken.safeTransfer(msg.sender, creatorShare);
        }
        
        // Transfer Platform Share (accumulate to fees or send specific wallet? 
        // Logic: Send to totalFees for owner to withdraw later, keeping pattern consistent)
        if (platformShare > 0) {
            totalFees += platformShare;
            // Alternatively, send directly to owner():
            // usdcToken.safeTransfer(owner(), platformShare);
            // I will adhere to the "Platform Owner withdraws via withdrawFees" pattern in doc.
        }
    }

    /**
     * @notice Set revenue split percentages
     * @param _platformShare Share for platform (e.g. 30)
     * @param _creatorShare Share for creator (e.g. 70)
     */
    function setRevenueSplit(uint256 _platformShare, uint256 _creatorShare) external onlyOwner {
        require(_platformShare + _creatorShare == 100, "Must equal 100");
        platformSharePercentage = _platformShare;
        creatorSharePercentage = _creatorShare;
    }

    // receive() removed as we move to USDC
}
