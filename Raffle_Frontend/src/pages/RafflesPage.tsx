...
import { GachaWheel } from '../features/raffle/components/GachaWheel';
import { Gift, Loader2, Ticket } from 'lucide-react';
import { Link } from 'react-router-dom';

export function RafflesPage() {
  const [activeTab, setActiveTab] = useState<'explore' | 'gacha'>('explore');
  ...
  return (
    <div className="w-full max-w-[100vw] overflow-x-hidden pb-safe md:pb-8">
      <div className="max-w-screen-lg mx-auto">
        ...
        {/* Cyber Tab Switcher */}
        <div className="tabs-cyber mb-6">
          <button
            onClick={() => setActiveTab('explore')}
            className={`tab-cyber-btn ${activeTab === 'explore' ? 'active' : ''}`}
          >
            Explore Campaigns
          </button>
          <button
            onClick={() => setActiveTab('gacha')}
            className={`tab-cyber-btn ${activeTab === 'gacha' ? 'active' : ''}`}
          >
            Gacha Spin Wheel
          </button>
        </div>
