...
import { ArrowDown, ExternalLink, Loader2, AlertCircle, RefreshCw, Settings2, ArrowLeft } from 'lucide-react';
...
export function SwapPage() {
  ...
  const [selectedChainId, setSelectedChainId] = useState<number>(() => {
    return import.meta.env.VITE_CHAIN_ID ? parseInt(import.meta.env.VITE_CHAIN_ID) : 8453;
  });
  ...
  const getSafeToken = (chainId: number, index: number, fallbackChainId: number = 8453): Token => {
    ...
    return (chainTokens && chainTokens.length > index)
      ? chainTokens[index]
      : { address: '0x0000000000000000000000000000000000000000', decimals: 18, symbol: 'ETH', logo: 'Ξ' };
  };
  ...
  useEffect(() => {
    ...
    try {
      createConfig({ integrator: 'crypto-disco-app' });
      configuredRef.current = true;
    } catch (e: unknown) {
      console.warn('[SwapPage] Li.Fi SDK init failed:', e instanceof Error ? e.message : String(e));
      configuredRef.current = true;
    }
  }, [wagmiConfig]);
  ...
  }, [amountIn, fromToken, toToken, selectedChainId, address, ecosystemSettings]);
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-[#050505] selection:bg-indigo-500/30">
        <div className="text-center glass-card p-12 max-w-md w-full border border-white/5 relative overflow-hidden group">
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
            AUTHENTICATE YOUR WALLET TO ACCESS SWAP ENGINE.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full max-w-[100vw] overflow-x-hidden pb-safe md:pb-8 pt-24 min-h-screen bg-[#050505] relative selection:bg-indigo-500/30 flex items-center justify-center px-4">
      ...
      <div className="w-full max-w-[420px] glass-card border border-white/5 bg-[#0a0a0c]/80 backdrop-blur-xl rounded-[2rem] p-6 relative z-10 flex flex-col gap-6">
        <div className="flex justify-between items-center pb-2 border-b border-white/5">
          <button
            onClick={() => navigate(-1)}
            className="p-2 bg-white/5 rounded-full hover:bg-white/10 transition-all text-slate-400 hover:text-white"
          >
            <ArrowLeft size={16} />
          </button>
          <h2 className="text-[12px] font-black text-white italic tracking-tighter flex items-center gap-2">
            DISCO <span className="text-indigo-500">SWAP PAGE</span>
          </h2>
        ...
        <div className="space-y-4">
          ...
          {/* Fee & Network Info */}
          <div className="px-1 space-y-1.5 pt-1">
