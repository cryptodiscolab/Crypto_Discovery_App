                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <div className="lg:col-span-3 space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group/input">
                                        ...
                                        <select value={task.platform} onChange={(e) => onUpdateTask(idx, 'platform', e.target.value)} className="w-full bg-white/5 border border-white/5 p-4 pl-12 rounded-2xl text-white text-[11px] font-black uppercase tracking-widest focus:border-purple-500/50 outline-none appearance-none cursor-pointer transition-all">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group/input">
                                        ...
                                        <select value={task.action} onChange={(e) => onUpdateTask(idx, 'action', e.target.value)} className="w-full bg-white/5 border border-white/5 p-4 pl-12 rounded-2xl text-white text-[11px] font-black uppercase tracking-widest focus:border-purple-500/50 outline-none appearance-none cursor-pointer transition-all">
                            ...
                            <div className="lg:col-span-5 space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input value={task.title} onChange={(e) => onUpdateTask(idx, 'title', e.target.value)} placeholder="Auto-filled based on action..." className="w-full bg-white/5 border border-white/5 p-4 rounded-2xl text-white text-sm font-black placeholder:text-slate-800 focus:border-purple-500/50 outline-none transition-all" />
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-emerald-500/60 uppercase tracking-widest px-1">Direct Link</label>
                                        <input value={task.link} onChange={(e) => onUpdateTask(idx, 'link', e.target.value)} placeholder={PLATFORMS[task.platform]?.domain || "https://..."} className="w-full bg-emerald-500/5 border border-emerald-500/10 p-3 rounded-xl text-emerald-400/80 text-[10px] font-mono focus:border-emerald-500/30 outline-none transition-all" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-blue-500/60 uppercase tracking-widest px-1">Target Account ID</label>
                                        <input value={task.target_id} onChange={(e) => onUpdateTask(idx, 'target_id', e.target.value)} placeholder="Username or ID" className="w-full bg-blue-500/5 border border-blue-500/10 p-3 rounded-xl text-blue-400/80 text-[10px] font-mono focus:border-blue-500/30 outline-none transition-all" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-4 grid grid-cols-2 gap-4 h-fit">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group/input">
                                        ...
                                        <input type="number" readOnly value={task.baseReward} className="w-full bg-white/5 border border-white/5 p-4 pl-10 rounded-2xl text-amber-400 font-black text-sm outline-none cursor-not-allowed" />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Min Tier</label>
                                    <select value={task.minTier} onChange={(e) => onUpdateTask(idx, 'minTier', Number(e.target.value))} className="w-full bg-white/5 border border-white/5 p-4 rounded-2xl text-white font-black text-[10px] uppercase tracking-widest focus:border-purple-500/50 outline-none cursor-pointer appearance-none">
                                ...
                                <div className="space-y-2 col-span-2 grid grid-cols-2 gap-3">
                                    ...
                                    >
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest">{task.requiresVerification ? 'VERIFY ON' : 'MANUAL'}</span>
                                    </button>
                                    <button
                                    ...
                                    >
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest">{task.isBaseSocialRequired ? 'IDENTITY ON' : 'OPEN'}</span>
                                    </button>
                                </div>
                            </div>
                        </div>
