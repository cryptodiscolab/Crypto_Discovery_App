                ...
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-3">
                            ...
                            <input type="number" value={newMinPoolUSD} onChange={(e) => onNewMinPoolUSDChange(e.target.value)} placeholder="5.00" className="w-full bg-black/40 border border-white/5 p-5 rounded-2xl text-white outline-none focus:border-indigo-500/50 transition-all font-mono" />
                        </div>
                        <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Min Reward (USD)</label>
                            <input type="number" value={newMinRewardUSD} onChange={(e) => onNewMinRewardUSDChange(e.target.value)} placeholder="0.01" className="w-full bg-black/40 border border-white/5 p-5 rounded-2xl text-white outline-none focus:border-indigo-500/50 transition-all font-mono" />
                        </div>
                    </div>
                    <button onClick={onUpdateEconomy} className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 py-6 rounded-[2rem] text-xs font-black uppercase tracking-[0.3em] text-white transition-all shadow-2xl shadow-indigo-500/20 active:scale-[0.98] mt-4">
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-8">
                        <div className="space-y-3">
                            ...
                            <input type="number" step="0.0001" value={newTokenPriceUSD} onChange={(e) => onNewTokenPriceUSDChange(e.target.value)} placeholder="0.1000" className="w-full bg-black/40 border border-white/5 p-6 rounded-[1.5rem] text-emerald-400 font-mono text-xl outline-none focus:border-emerald-500/50 transition-all" />
                        </div>
                        <button onClick={onSchedulePrice} className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 py-6 rounded-[2rem] text-xs font-black uppercase tracking-[0.3em] text-white transition-all shadow-2xl shadow-emerald-500/20 active:scale-[0.98]">
