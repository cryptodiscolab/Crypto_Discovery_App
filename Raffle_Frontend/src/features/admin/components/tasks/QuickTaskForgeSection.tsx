        ...
        <div className="bg-[#121214] p-5 rounded-2xl border border-white/5 space-y-4 text-left">
            ...
            <div className="space-y-3">
                <div className="space-y-1">
                    <label className="block label-native text-slate-500 px-1">Activity Template (From DB)</label>
                    <select
                        onChange={(e) => {
                        ...
                        className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 content-native text-white outline-none focus:border-indigo-500/50 transition-colors cursor-pointer"
                    >
                ...
                <input
                    ...
                    className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 content-native text-white outline-none focus:border-indigo-500/50"
                />
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        <label className="block label-native text-slate-500 px-1">XP Points</label>
                        <input
                            type="number"
                            placeholder="100"
                            value={dailyPoints}
                            onChange={e => onDailyPointsChange(e.target.value)}
                            className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 value-native text-indigo-400 outline-none"
                        />
                    </div>
                    <div className="space-y-1">
                        <label className="block label-native text-slate-500 px-1">Cooldown</label>
                        <select
                            value={dailyCooldown}
                            onChange={e => onDailyCooldownChange(e.target.value)}
                            className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 content-native text-white outline-none cursor-pointer"
                        >
                ...
                <div className="space-y-2">
                    <div className="flex items-center gap-4 px-2 bg-[#0a0a0c] py-3 rounded-xl border border-white/5">
                        <label className="flex items-center gap-2 cursor-pointer group flex-1">
                            ...
                            <span className="label-native text-slate-500 group-hover:text-white transition-colors">Requires Human verification</span>
                        </label>
                        <div className="h-4 w-px bg-white/5" />
                        <select value={dailyMinTier} onChange={e => onDailyMinTierChange(Number(e.target.value))} className="bg-transparent label-native text-indigo-500 outline-none cursor-pointer">
                    ...
                    {/* IDENTITY GUARD */}
                    <div
                        className={`flex items-center justify-between px-3 py-2.5 rounded-xl border transition-all cursor-pointer select-none ${dailyIsBaseSocialRequired ? 'bg-blue-600/10 border-blue-500/30 shadow-lg shadow-blue-500/5' : 'bg-[#0a0a0c] border-white/5 hover:border-white/10'}`}
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`label-native ${dailyIsBaseSocialRequired ? 'text-blue-400' : 'text-slate-500'}`}>Identity Guard (v3.42.0)</span>
                        </div>
            ...
            <AdminTransactionButton
                ...
                className="w-full py-4 bg-indigo-600/20 hover:bg-indigo-600/30 border border-indigo-500/30 text-indigo-400 rounded-xl label-native transition-all"
            />
        </div>
    );
}
