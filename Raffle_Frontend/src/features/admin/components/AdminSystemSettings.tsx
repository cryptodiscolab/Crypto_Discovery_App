...
export default function AdminSystemSettings() {
    ...
    const fetchPointSettings = async () => {
        try {
            ...
            toast.error("Failed to sync DB data: " + (err.message || String(error)));
        }
    };

    // --- Action Handlers ---

    const handlePointChange = (id: string, field: string, newValue: string | number | boolean) => {
        setPointSettings(prev => prev.map(item => item.id === id ? { ...item, [field]: field === 'points_value' ? parseInt(newValue as string) || 0 : newValue } : item));
    };
    ...
    const savePoints = async () => {
        ...
        try {
            ...
            const err = error as { message?: string };
            toast.error('Failed to save points: ' + (err.message || String(error)), { id: tid });
        } finally { setSaving(false); }
    };

    const handleThresholdChange = (id: string, field: string, newValue: string | number) => {
        setSbtThresholds(prev => prev.map(item => item.id === id ? { ...item, [field]: (field === 'min_xp' || field === 'level') ? parseInt(newValue as string) || 0 : newValue } : item));
    };
    ...
    const saveThresholds = async () => {
        ...
        try {
            ...
            const err = error as { message?: string };
            toast.error('Failed to save thresholds: ' + (err.message || String(error)), { id: tid });
        }
        finally { setSaving(false); }
    };
    ...
    const saveTierConfig = async (silent = false) => {
        ...
        try {
            ...
            const err = error as { message?: string };
            if (!silent) toast.error('Failed to save tier config: ' + (err.message || String(error)), { id: tid ?? undefined });
            throw error;
    ...
    const handleManualOverride = async () => {
        ...
        try {
            ...
            const err = error as { message?: string };
            toast.error('Override failed: ' + (err.message || String(error)), { id: tid });
        }
        finally { setSaving(false); }
    };
    ...
    const handleResetSeason = async () => {
        ...
        try {
            ...
            const errMsg = error.shortMessage || error.message || String(e);
    ...
    const issueSubname = async (user: EligibleUser, label: string) => {
        ...
        try {
            ...
            const err = error as { message?: string };
            toast.error('ENS Error: ' + (err.message || String(error)), { id: tid });
        }
        finally { setSaving(false); }
    };
