...
export function EconomyMetrics() {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 text-left">
            {/* 1. Revenue Card */}
            <div className="glass-card p-6 bg-indigo-500/5 border border-white/5 rounded-3xl hover:border-indigo-500/20 transition-all group">
                <div className="flex justify-between items-start mb-4">
                    <div className="p-3 bg-indigo-500/10 rounded-2xl text-indigo-400 group-hover:scale-110 transition-transform">
                        <DollarSign className="w-6 h-6" />
                    </div>
                    <span className="label-native px-2 py-1 rounded-full bg-indigo-500/10 text-indigo-400">Revenue</span>
                </div>
                <h4 className="text-3xl font-black text-white mb-1 font-mono">
                    {!stats ? '----' : `$${stats?.totalRevenueUSDC || '0.00'}`}
                </h4>
                <p className="label-native text-slate-500 mt-1">Gross Listing Revenue (USDC)</p>
            </div>
            ...
            <div className="glass-card p-6 bg-indigo-500/5 border border-white/5 rounded-3xl hover:border-indigo-500/20 transition-all group">
                <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="label-native px-2 py-1 rounded-full bg-indigo-500/10 text-indigo-400">P&L</span>
                </div>
                <h4 className="text-3xl font-black text-white mb-1 font-mono">
                    {!stats ? '----' : `$${stats?.netProfit || '0.00'}`}
                </h4>
                <p className="label-native text-slate-500 mt-1">Estimated Net Profit (70% Margin)</p>
            </div>
            ...
            <div className="glass-card p-6 bg-indigo-500/5 border border-white/5 rounded-3xl hover:border-indigo-500/20 transition-all group">
                <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="label-native px-2 py-1 rounded-full bg-indigo-500/10 text-indigo-400">Engagement</span>
                </div>
                <h4 className="text-3xl font-black text-white mb-1 font-mono">
                    {!stats ? '----' : ((stats?.communityXp || 0) / 1000).toFixed(1) + 'k'}
                </h4>
                <p className="label-native text-slate-500 mt-1">Total XP Distributed to Users</p>
            </div>

            {/* 4. System Health / Verifier Status */}
            <div className={`glass-card p-6 border transition-all group rounded-3xl ${hasVerifierRole ? 'bg-indigo-500/5 border-white/5 hover:border-indigo-500/20' : 'bg-red-500/5 border-red-500/20 shadow-red-500/10 shadow-lg'}`}>
                <div className="flex justify-between items-start mb-4">
                    <div className={`p-3 rounded-2xl group-hover:scale-110 transition-transform ${hasVerifierRole ? 'bg-indigo-500/10 text-indigo-400' : 'bg-red-500/10 text-red-400'}`}>
                        {hasVerifierRole ? <ShieldCheck className="w-6 h-6" /> : <AlertCircle className="w-6 h-6 animate-pulse" />}
                    </div>
                    <span className={`label-native px-2 py-1 rounded-full ${hasVerifierRole ? 'bg-indigo-500/10 text-indigo-400' : 'bg-red-500/10 text-red-400'}`}>
                        System Health
                    </span>
                </div>
                ...
                <p className="label-native text-slate-500 mt-1">
            ...
            <button
                onClick={fetchStats}
                className={`md:col-span-3 lg:col-span-4 py-3 border rounded-2xl label-native flex items-center justify-center gap-2 transition-all mt-2 ${!stats
