...
export function SBTGallery({ walletAddress }: { walletAddress?: string }) {
    ...
    const fetchWeights = async () => {
        const { data } = await supabase
            ...
            .maybeSingle();
        if (data?.value) setWeights(data.value as Record<string, number>);
    };
    ...
    const getTierWeightLabel = (tierId: number) => {
