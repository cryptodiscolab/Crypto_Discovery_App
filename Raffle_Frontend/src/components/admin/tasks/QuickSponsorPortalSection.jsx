        ...
        <div className="bg-[#121214] p-5 rounded-2xl border border-white/5 space-y-4 text-left">
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-1 gap-3">
                    <input placeholder="App/Community Name" value={sponsorTitle} onChange={e => onSponsorTitleChange(e.target.value)} className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-indigo-500/50" />
                    <input placeholder="Destination URL (Farcaster/X/App)" value={sponsorLink} onChange={e => onSponsorLinkChange(e.target.value)} className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-indigo-500/50" />
                    <input placeholder="Contact Email (Verification Only)" value={sponsorEmail} onChange={e => onSponsorEmailChange(e.target.value)} className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 text-xs text-white outline-none focus:border-indigo-500/50" />
                </div>

                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        <label className="text-[7px] font-black text-slate-700 uppercase px-1">Total Reward Slots</label>
                        <input type="number" value={sponsorTotalClaims} onChange={e => onSponsorTotalClaimsChange(e.target.value)} className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 text-xs text-white font-black outline-none" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[7px] font-black text-slate-700 uppercase px-1">USD Value Per User</label>
                        <input type="number" step="0.01" value={sponsorRewardPerUser} onChange={e => onSponsorRewardPerUserChange(e.target.value)} className="w-full bg-[#0a0a0c] border border-white/5 rounded-xl px-4 py-3 text-xs text-green-500 font-black outline-none" />
                    </div>
                </div>
            </div>
            ...
            <Transaction calls={buildSponsorCall()} onSuccess={handleTxSuccess}>
                <TransactionButton
                    ...
                    className="w-full py-4 bg-white hover:bg-slate-100 text-black rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl disabled:opacity-30 disabled:grayscale transition-all"
