...
### 2. Verified Infrastructure Reference (v3.59.1)
| Key | Value |
|---|---|
...
| PRD Version | `v3.59.1` (Ecosystem Infrastructure Hardening) |
...
- **🚨 ZERO-HARDCODE ADDRESSING MANDATE (v3.59.1)**: All contract addresses MUST be pulled from `.env` via `VITE_*` keys. Hardcoded addresses in `abis_data.txt` or component files are strictly forbidden to prevent environment drift.
...
- [ ] **🚀 REFERRAL & IDENTITY SYNC (v3.42.8)**:
