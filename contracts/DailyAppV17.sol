...
contract DailyAppV17 is UUPSUpgradeable {
    ...
    uint256 public constant REFERRAL_ACTIVATION_TASK_COUNT = 3; // Kept for legacy compatibility / stats
    ...
    uint256 public constant MAX_ADMIN_XP_PER_USER_PER_CALL = 500000;
    ...
    // ── V17 New Storage ──
    mapping(address => bool) public hasReferralBeenPaid;
    ...
    function _creditXp(address user, uint256 amount, XpSource source) internal validAddress(user) {
        ...
        // V17: Vest referral bonus when user reaches 500 XP threshold
        address referrer = referrerOf[user];
        if (referrer != address(0) && !userStats[referrer].isBlacklisted) {
            if (!hasReferralBeenPaid[user] && userStats[user].points >= 500) {
                hasReferralBeenPaid[user] = true;
                // Recursive call, bounded by referral tree depth (prevent infinite loop by setting flag first)
                _creditXp(referrer, baseReferralReward, XpSource.REFERRAL);
        ...
        onlyRole(ADMIN_ROLE)
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            if (amounts[i] > MAX_ADMIN_XP_PER_USER_PER_CALL) revert RateLimitReached();
    ...
    function doTask(uint256 taskId, address referrer) external notBlacklisted whenNotPaused {
        ...
        stats.tasksForReferralProgress++; // Kept for statistical tracking
        ...
        // V17 removed referral vesting here since it is now handled inside _creditXp()
