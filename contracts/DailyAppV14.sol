...
/**
 * @title DailyAppV14
 * @notice Task-based gamification with decimal-aware multi-token sponsorship rewards
 * @dev V14 fixes decimal mismatch bug: all thresholds now in USDC 6-decimal base
 * @author DailyApp Team
 */
contract DailyAppV14 is ERC721, AccessControl, Pausable, ReentrancyGuard {
    ...
    uint256 public sponsorshipPlatformFee = 2 * 10**6; // $2 USDC
    bool public autoApproveSponsorship = true; 
    uint256 public minRewardPoolValue = 2 * 10**6;  // V14: $2 in 6-dec
    uint256 public rewardPerClaim = 2 * 10**5;      // V14: $0.20 in 6-dec
    ...
    struct SponsorRequest { 
        ...
        uint256 timestamp;
        address rewardToken;    // V14: token used for reward pool
        uint8 rewardDecimals;   // V14: decimals of reward token
    }

    // V14: Token configuration for decimal-aware operations
    struct TokenConfig {
        bool allowed;
        uint8 decimals;
        string symbol;
    ...
    mapping(address => mapping(address => uint256)) public claimableRewards; // V14: user => token => amount
    ...
    // V14: Token config replaces simple bool whitelist
    mapping(address => TokenConfig) public tokenConfigs;
    // Legacy compat: kept for read-only checks
    ...
    event RewardsClaimed(address indexed user, address indexed token, uint256 amount, uint256 timestamp); // V14.1 Audit Support
        ...
        validAddress(initialOwner)
    {
        ...
        // V14: Init token configs with decimals
    ...
    function batchMigrateUsers(
        ...
        for(uint i = 0; i < len; i++) {
            ...
            // Increment userCount if they are new
            if (!_hasJoined[u]) {
                _hasJoined[u] = true;
                userCount++;
            }
            ...
            // Re-mint the NFT if they don't have it on this contract yet
    ...
    /**
     * @notice V14: Admin whitelist a payment token with decimals.
     * @param _token Token address (address(0) = ETH)
     * @param _status true = allowed, false = removed
     * @param _decimals Token decimals (6 for USDC, 18 for ETH)
     * @param _symbol Token symbol for display
     */
    ...
    function buySponsorshipWithToken(
        ...
        req.rewardToken = _paymentToken;       // V14
        req.rewardDecimals = tokenCfg.decimals; // V14
        ...
        notBlacklisted 
    {
        ...
        // V14: Sponsored Task Logic — Decimal-Aware Pull Reward
        if (task.sponsorshipId > 0) {
            ...
            if (currentProgress % tasksForReward == 0) {
                ...
                // V14: Convert rewardPerClaim (6-dec base) to native token decimals
                ...
                if (req.rewardPool >= rewardNative) {
                    ...
                    claimableRewards[msg.sender][req.rewardToken] += rewardNative;
                }
            }
        }
     ...
     */
    /**
     * @notice V14: Admin add claimable reward for a specific token
     */
    ...
    /**
     * @notice V14: Claim accumulated rewards for a specific token.
     * @param _token The reward token address. address(0) for native ETH.
     */
    ...
    /**
     * @notice V13 Signature-Based Sync
     * Allows syncing off-chain DB XP directly to on-chain by passing a backend signature
     */
    function syncOffchainXP(uint256 totalDbXp, uint256 deadline, bytes calldata signature) external whenNotPaused nonReentrant notBlacklisted {
        ...
        if (totalDbXp <= maxSyncedDbXp[msg.sender]) revert Unauthorized(); // Prevent replay and negative sync

        // 1. Verify Signature
        bytes32 messageHash = keccak256(abi.encodePacked(msg.sender, totalDbXp, deadline));
        bytes32 ethSignedMessageHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", messageHash));
        
        address signer = ethSignedMessageHash.recover(signature);
        if (signer != verifierWallet) revert Unauthorized();

        // 2. Calculate Missing XP
        uint256 diff = totalDbXp - maxSyncedDbXp[msg.sender];
        
        // 3. Update Sync State
        maxSyncedDbXp[msg.sender] = totalDbXp;

        // 4. Update XP Balances
        userStats[msg.sender].points += diff;
        unsyncedPoints[msg.sender] += diff; // Accumulate for MasterX propagation
    ...
    // --- V14: DECIMAL NORMALIZATION HELPER ---

    /**
     * @notice Convert an amount between different decimal bases.
     * @param amount The raw amount to convert
     * @param fromDecimals Source decimals (e.g. 18 for ETH)
     * @param toDecimals Target decimals (e.g. 6 for USDC comparison)
     */
