...
- [x] Production X OAuth long-loading fallback deployed: `/oauth-callback` now notifies the opener through `postMessage`, `localStorage`, and `BroadcastChannel` so a successful X authorization cannot leave the profile page stuck in `Opening X sign-in...` when browser isolation drops the direct opener message.
...
- [x] Re-audited wallet `0x08452c1bdAa6aCD11f6cCf5268d16e2AC29c204B` X linking long-loading report on 2026-06-06. Commit `ca06641` adds a redundant callback delivery channel: parent listens to same-origin `postMessage`, provider-scoped `localStorage` storage events, and `BroadcastChannel`; callback writes/posts the same `OAUTH_SUCCESS` or `OAUTH_ERROR` payload to all three paths before closing the popup.
- [x] Deployed commit `ca06641` to production Vercel. Live production now serves index `/assets/index-iy8arYri.js`, OAuth callback chunk `OAuthCallbackPage-vC6zUZFL.js`, profile chunk `ProfilePage-DCUTxwOj.js`, and OAuth hook chunk `useOAuth-CfX9NVuL.js`; live chunks contain `BroadcastChannel` and `crypto-disco:oauth:result`, and do not contain the old `OAuth popup closed by user` error string.
...
- [ ] Manual interactive QA remains required for a real X OAuth account: hard refresh production, connect wallet `0x08452c1bdAa6aCD11f6cCf5268d16e2AC29c204B`, click `Authorize app` in the X popup, sign the wallet message, then verify `user_profiles.twitter_id/twitter_username`, `user_activity_logs` `X Link`, and UI `CONNECTED` state.
