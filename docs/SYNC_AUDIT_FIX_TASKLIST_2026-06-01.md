...
- [x] Fixed X social verification regression: Supabase OAuth linking now uses canonical provider `twitter`, frontend/API normalize task platform alias `x -> twitter`, Twitter task fallback now fetches `twitter_id` even when Farcaster FID exists, and social verification payloads restore flat `userId` compatibility for the verification server.
...
- [x] Re-audited user-reported X verification failure on 2026-06-05: [useOAuth.ts](file:///e:/Disco%20Gacha/Disco_DailyApp/Raffle_Frontend/src/hooks/useOAuth.ts) was sending a casted `x` OAuth provider even though Supabase expects `twitter`; fixed it to call `signInWithOAuth({ provider: 'twitter' })` while keeping app-facing X labels unchanged.
