# ANTIGRAVITY — Codex NATIVE PROTOCOL (v3.64.23-Hardened)
...
`> re-read skills` = reset, `> sync end to end` = full ecosystem sync. Protocol: 🟢 Healthy (v3.64.23).

---

## 🛡️ SECURE INSTALL (v3.63.5)
`npm install --ignore-scripts` or `npm ci`. Supply-chain safe.

### SOT HIERARCHY
Contracts > Supabase `system_settings`/`point_settings` > PRD/ > `.cursorrules` + `CLAUDE.md` > DESIGN.md > SKILL.md.

---

## Rule 0 — THINKING BUDGET + SYMBOLIC (⛽ Max Token Savings).
...
21. ❌ delete `last_seen_at`. 22. Post-fix doc sync: PRD, AGENTS, WORKSPACE_MAP, ROADMAP, IMPLEMENTATION, .cursorrules, SKILL.md.
...
**Windows Binary Path**: `.\.bin\rtk.exe <subcommand> <args>` (prioritas pertama)
**Global Path** (jika tersedia di PATH): `rtk <subcommand> <args>`

**Protocol**:
1. `📄>.rtk/RTK.md` + `📄>.rtk/SYMBOLIC.md` — protocol + symbolic lexicon
2. `📄>.rtk/filters.toml` — 26 filter kustom
3. `.\.bin\rtk.exe trust` — sekali setelah clone/update
4. Semua command `▶️rtk <cmd>` — tanpa kecuali
5. `📄>f` WAJIB `rtk read --level aggressive --max-lines 80 <f>` (50-70% savings)
6. `📄>f` (>500 baris) → `rtk smart <f>` (80-95% savings)
7. Fallback raw hanya jika RTK tidak bisa, sebutkan alasan
8. Sebelum finalisasi: `.\.bin\rtk.exe gain`
9. Target: 90% savings
...
*Antigravity: Absolute Honesty. Token Hygiene. No Paper Protocol. Zero-Trust. Nexus War War Room Mode: Active. v3.64.23 LOCKED.*
...
*Audit Precision Hardened: 2026-05-14. v3.64.23 MILLISECOND PRECISION & MULTI-ASSET.*
*Anti-Negligence Hook Activated: 2026-05-18. v3.64.23 AUTOMATED SCANNING & RTK INTEGRATED.*
*Live Agent Delegation Dashboard Activated: 2026-05-20. v3.64.23 HIERARCHICAL TASK FEED & 100% BUILD SYNCHRONIZATION.*
