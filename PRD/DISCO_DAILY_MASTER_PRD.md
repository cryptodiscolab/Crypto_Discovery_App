...
|-------|-----------|-----------------|----------------|
| **On-Chain** | `claimDailyBonus()` di DailyApp contract | `/api/user-bundle` action `daily-claim` | Receipt verify → baca `userStats(wallet)` → mirror `last_onchain_xp`/streak/tier → `fn_increment_xp` untuk delta positif |
...
graph TB
    ...
    subgraph "⚡ API Layer (Vercel Serverless - Hardened v3.61.0)"
        ...
        UB["user-bundle.ts<br/>handleDailyClaim (Receipt-Verified On-Chain)"]
...
- **Root Dev-Tool Audit**: Pruned unused Airnode npm packages, removed unused Hardhat toolbox/Ignition/network-helper/gas-reporter/coverage/TypeChain dependencies, replaced toolbox loading with explicit Hardhat plugins, upgraded `hardhat` to `2.28.6`, upgraded `hardhat-contract-sizer` to `2.10.1`, and removed stale Airnode/bn.js overrides. Root dependency count is reduced to 651 packages and `npm audit` is reduced from 80 findings to **14 low** findings with **0 moderate/high/critical** findings; the remaining low findings are isolated to Hardhat 2 / verify / upgrades transitive packages and require a breaking Hardhat 3 migration to fully eliminate.
