...
graph TB
    ...
    subgraph "ΓÜí API Layer (Vercel Serverless - Hardened v3.61.0)"
    ...
    subgraph "≡ƒùä∩╕Å Database (Supabase)"
        DT["daily_tasks"]
    ...
    subgraph "Γ¢ô∩╕Å Blockchain (Base Sepolia)"
...
| Aspek | ≡ƒƒª On-Chain Tasks | ≡ƒƒ⌐ Off-Chain Tasks |
|---|---|---|
...
| **Verifikasi Sosial** | Ya ΓÇö via `useVerification.js` | Tidak ΓÇö klaim langsung |
| **Pemeriksaan Selesai** | `hasCompletedTask(user, taskId)` on-chain | `user_task_claims` lookup di DB |
| **Klaim XP** | `doTask()` ΓåÆ `awardTaskXP()` ΓåÆ `/api/tasks/verify` | `useVerifiedAction` ΓåÆ `/api/tasks-bundle?action=claim` |
| **XP Sync Method** | `fn_increment_xp` via tasks-bundle | `fn_increment_xp` via tasks-bundle |
| **Reward Source** | `point_settings` (dynamic key) | `point_settings` ΓåÆ fallback `daily_tasks.xp_reward` |
| **Sponsored Grouping** | Ya ΓÇö `sponsorshipId` grouping | Tidak |
| **Tier Gating** | Ya ΓÇö `minTier` on-chain | Tidak secara langsung |
| **Identity Guard** | Ya ΓÇö `isBaseSocialRequired` | Ya ΓÇö `is_base_social_required` column |
| **Unique Constraint** | `hasCompletedTask` (on-chain boolean) | `user_task_claims` UNIQUE(wallet, task_id) |

```mermaid
flowchart LR
    subgraph "On-Chain Path"
        A1[User Sees TaskRow] --> A2[Click ΓåÆ Open Link + doTask on-chain]
        A2 --> A3[Wait 30s Anti-Fraud]
        A3 --> A4[Click Verify ΓåÆ EIP-191 Sign]
    ...
    subgraph "Off-Chain Path (v3.47.1 Two-Step Flow)"
        B1[User Sees TaskList Card] --> B2["Step 1: Click 'GO TO TASK' ΓåÆ window.open(task_link)"]
...
### 3.1 Contract Addresses (Base Sepolia ΓÇö 84532)
...
> [!WARNING]
> Mainnet addresses are `[RESERVED]` ΓÇö NEVER use Sepolia addresses on Mainnet labels.

### 3.2 DailyApp V13.2 ΓÇö Task-Related Functions

| Function | Type | Signature | Purpose |
|---|---|---|---|
| `getTask` | `view` | `(uint256 taskId) ΓåÆ (baseReward, isActive, cooldown, minTier, title, link, createdAt, requiresVerification, sponsorshipId)` | Membaca detail tugas |
| `getTasksInRange` | `view` | `(uint256 from, uint256 to) ΓåÆ Task[]` | Batch baca seluruh tugas |
| `nextTaskId` | `view` | `() ΓåÆ uint256` | Total tugas yang terdaftar |
| `doTask` | `write` | `(uint256 taskId, address referrer)` | Registrasi intent/cooldown user |
| `hasCompletedTask` | `view` | `(address user, uint256 taskId) ΓåÆ bool` | Cek apakah tugas sudah selesai |
| `isTaskVerified` | `view` | `(address user, uint256 taskId) ΓåÆ bool` | Cek status verifikasi |
| `userStats` | `view` | `(address user) ΓåÆ (points, totalTasksCompleted, referralCount, currentTier, tasksForReferralProgress, lastDailyBonusClaim, isBlacklisted)` | Statistik user |
| `userSponsorshipProgress` | `view` | `(address user, uint256 sponsorshipId) ΓåÆ uint256 count` | Progress sponsorship mission |
| `claimableRewards` | `view` | `(address user) ΓåÆ uint256` | Reward yang bisa diklaim |
| `claimRewards` | `write` | `()` | Klaim akumulasi reward |
| `lastActivityTime` | `view` | `(address user) ΓåÆ uint256` | Timestamp aktivitas terakhir |
...
Import: src/lib/contracts.js (Proxy pattern ΓÇö anti Rollup AST crash)
```
...
### 4.1 `daily_tasks` ΓÇö Off-Chain Task Definitions
...
| `target_id` | `text` | ID target sosial (FID, tweet ID, etc.) ΓÇö Sybil prevention |
| `task_type` | `text` | `social`, `regular`, `system` ΓÇö Sistem tasks difilter dari UI |
...
### 4.2 `user_task_claims` ΓÇö Claim History (One-Time Per Task)
...
| **UNIQUE** | | `(wallet_address, task_id)` ΓÇö Mencegah duplikasi |

### 4.3 `point_settings` ΓÇö Dynamic XP Configuration (Zero-Hardcode)
...
### 4.4 `user_profiles` ΓÇö Core User Identity

| Column | Type | Task-Related |
|---|---|---|
| `wallet_address` | `text` (PK) | Γ£à Join key untuk claims |
| `total_xp` | `integer` | Γ£à Updated via `fn_increment_xp` setelah klaim |
| `tier` | `integer` | Γ£à Tier gating untuk tugas |
| `is_base_social_verified` | `boolean` | Γ£à Identity guard |
| `neynar_score` | `integer` | Γ£à Reputation gating |
| `referred_by` | `text` | Γ£à Referral dividend tracking |
| `referral_bonus_paid` | `boolean` | Γ£à 500 XP milestone vesting |
| `last_seen_at` | `timestamp` | Γ£à Leaderboard sync |

### 4.5 `user_activity_logs` ΓÇö Audit Trail
...
### 4.6 `fn_increment_xp` ΓÇö Atomic XP RPC Function

```sql
-- Signature
fn_increment_xp(p_wallet TEXT, p_amount INT) ΓåÆ void
...
### 4.7 `v_user_full_profile` ΓÇö Unified View
...
### 4.8 `telegram_chat_history` ΓÇö Conversational Memory (v3.56.4)
...
### 4.9 `nexus_agent_reports` ΓÇö Intelligence Feed
...
### 5.2 tasks-bundle.js ΓÇö Handler Map
...
2. Get XP reward from point_settings (dynamic) ΓåÆ fallback to daily_tasks.xp_reward
3. Check Sybil: target_id uniqueness + global task_id uniqueness
4. Insert into user_task_claims (UNIQUE constraint)
5. Call fn_increment_xp(wallet, base_xp) ΓÇö scaling handled in DB
...
sequenceDiagram
    ...
    FE->>FE: registerTaskStart(taskId) ΓåÆ start 30s timer
    ...
    FE->>SC: refetch hasCompletedTask ΓåÆ true
...
sequenceDiagram
    ...
    Note over U: STEP 1 ΓÇö User Clicks "GO TO TASK" (v3.47.1)
    ...
    Note over U: STEP 2 ΓÇö After 15s, User Clicks "CLAIM REWARD"
    ...
    alt Neynar Score too low
        ...
        API->>DB: getTaskReward(task_id) ΓåÆ point_settings lookup
        ...
        TL->>TL: Optimistic: add to userClaims ΓåÆ task disappears
        TL->>TL: setTimeout(fetchData, 1500ms) ΓåÆ server sync
    end
```
...
flowchart TD
    ...
    V1 --> V2[getTaskReward ΓåÆ point_settings]
...
```
Final_XP = MAX(5, ROUND(Base_XP ├ù G ├ù I ├ù U))
```

| Variable | Formula | Range | Purpose |
|---|---|---|---|
| **G** (Global) | `1.5 / (1 + logΓéüΓéÇ(users/1000 + 1))` | ~0.75 ΓÇô 1.5 | Anti-inflasi seiring pertumbuhan user |
| **I** (Individual) | `MAX(0.5, 1.0 - (xp/20000))` | 0.5 ΓÇô 1.0 | Anti-whale; pemain lama dapat lebih sedikit |
| **U** (Underdog) | `1.1 if tier Γëñ Silver` | 1.0 ΓÇô 1.1 | +10% catch-up untuk tier rendah |
| **Floor** | `MAX(5, ...)` | ΓëÑ 5 | Minimum reward terjamin |
...
flowchart TD
    ...
    Type -- "UUID (Supabase)" --> DB1[daily_tasks ΓåÆ get platform + action_type]
...
2. **Passive Dividend**: Berikan 10% ├ù `base_xp` ke Tier 1 referrer
...
flowchart TD
    ...
    C1 -- No --> Block1["≡ƒÜ½ Toast: Connect Wallet"]
    C1 -- Yes --> C2{On-Chain: minTier check}
    C2 -- Locked --> Block2["≡ƒöÆ Badge: LVL X Required"]
    C2 -- Pass --> C3{is_base_social_required?}
    C3 -- Yes --> C4{user.is_base_social_verified?}
    C4 -- No --> Block3["≡ƒ¢í∩╕Å Badge: IDENTITY GUARD<br/>Button: VERIFY IDENTITY TO UNLOCK"]
    C4 -- Yes --> C5{min_neynar_score?}
    C3 -- No --> C5
    C5 -- Score Low --> Block4["ΓÜá∩╕Å Badge: LOW REPUTATION"]
    C5 -- Pass --> C6{Already Completed?}
    C6 -- Yes --> Block5["Γ£à return null (disappear)"]
    C6 -- No --> Allow["Γ£à Task Available"]
```
...
1. User mengunjungi Profil ΓåÆ Klik "LINK BASE SOCIAL"
...
```javascript
// TasksPage.jsx ΓÇö TaskRow component
...
```javascript
// TaskList.jsx ΓÇö handleClaim success/error handler
...
// Force sync: add to local claims ΓåÆ task disappears instantly
...
- UI: `OffersList.jsx` ΓåÆ `CampaignCard` component

### 12.2 Join Flow

```
User ΓåÆ Click "JOIN MISSION" ΓåÆ EIP-191 Sign ΓåÆ POST /api/campaigns {action: 'join'}
ΓåÆ Backend validates signature ΓåÆ Insert campaign_participants ΓåÆ Success
```
...
| Duration | Permanent or expires_at | `start_at` ΓåÆ `end_at` window |
...
- [x] **Disappearing Tasks**: Completed tasks return `null` (not "DONE" badge) ΓÇö verified di kedua jalur.
...
- [x] **Dual Pipeline Routing Fix**: `useVerifiedAction.js` ΓÇö `claim_task` actions ALWAYS route to `/api/tasks-bundle`, never to Verification Server. `isSocialVerify` only true for non-claim social verification actions (v3.51.1).
